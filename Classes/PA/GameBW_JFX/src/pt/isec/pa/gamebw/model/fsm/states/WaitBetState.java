...
public class WaitBetState extends GameBWStateAdapter {
    ...
    public BetResult bet(int nr_balls) {
        ...
        switch (ball) {
            case WHITE -> {
                data.betWon();
                changeState(data.bagIsEmpty() ? GameBWState.INFO : GameBWState.WAIT_BET); //mudado de begin para info
            ...
            case NONE -> changeState(GameBWState.INFO); //mudado de begin para info
        }
        return BetResult.ERROR;
    }
    ...
    @Override
    public void end() {
        changeState(GameBWState.INFO);
    } //mudado de begin para info
